...
class AppHealthTests(unittest.TestCase):
    ...
    def test_root_page_returns_unified_briefings_home_and_composer(self) -> None:
        client = TestClient(create_app(settings=AppSettings()))
        for path in ("/", "/app/", "/app/index.html"):
            with self.subTest(path=path):
                ...
                self.assertIn("Advisor Home", html)
                ...
                self.assertIn("Create client briefing template", html)
                self.assertIn("Create advisor review template", html)
                ...
                self.assertIn("Developer / QA tools", html)
                ...
                self.assertIn("Briefing Templates", first_screen)
                self.assertIn("Generated Reports", first_screen)
                self.assertIn("Create or manage templates", first_screen)
                self.assertIn("Developer / QA tools", first_screen)
                self.assertNotIn("Choose a conversation", first_screen)
                self.assertNotIn("Manage briefing workflows", first_screen)
                self.assertNotIn("Present / view reports", first_screen)
                self.assertNotIn("home-primary-actions", first_screen)
