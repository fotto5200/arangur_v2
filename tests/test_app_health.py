    ...
    def test_root_page_returns_report_element_spec_composer(self) -> None:
        ...
        for path in ("/", "/app/", "/app/index.html"):
            with self.subTest(path=path):
                ...
                self.assertIn("Current local set", response.text)
                self.assertIn("Preview Client Briefing Set", response.text)
                self.assertIn("Preview Advisor Review Set", response.text)
                self.assertIn("Copy current spec set", response.text)
                self.assertIn("Download current spec set", response.text)
                self.assertIn("Serialized spec-set data", response.text)
                self.assertIn('<details id="serialized-spec-details">', response.text)
                self.assertIn("Restore from exported spec set", response.text)
                self.assertIn('<details id="restore-spec-details">', response.text)
                self.assertIn("Deterministic sample previews", response.text)
                self.assertIn("Open deterministic sample Client preview", response.text)
                self.assertIn("Open deterministic sample Advisor preview", response.text)
                self.assertIn("Current local set preview", response.text)
                self.assertIn("No rendered preview available yet for this spec.", response.text)
