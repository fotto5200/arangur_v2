...
- Next choose whether to resume Docker Compose private-demo packaging so Postgres-backed briefing spec-set save/load can be exercised in a local stack, or do browser visual QA against the new secondary backend panel once a database-backed app is running. Do not implement charts, live data, production auth, production reporting, or generated reports from saved spec sets unless explicitly authorized.
