...
- Next recommended path: Frank should review the polished `docs/product/report_mockups/attribution_v1/` and choose one of three local product paths: make a targeted report-element content revision, continue broader v2 content polish, or build Scenario by Lens aggregation from existing full-revaluation rows and complete synthetic lens assignments. Advisor UI/report wiring, generated-report integration, and private-server deployment planning remain later separate decisions.
