...
- Next ask Frank/ChatGPT for the next bounded product prompt. A likely next product-path batch is a local private-demo browser rehearsal using the updated walkthrough/checklist, or a narrowly scoped durable local/private-demo artifact persistence decision if explicitly authorized. Do not implement charts, live data, production auth, production reporting, generated report history, full report library UI, Caddy, Lightsail, Cloudflare, DNS, or public deployment changes unless explicitly authorized.
...
- Acceptance criteria: Implemented baseline: default Client Briefing content now opens with conversation framing and sequences Portfolio Status, Cash & Liquidity, Concentration Watch, Scenario Sensitivity, Data Confidence, Discussion Prompts, and a restrained demo note; default Advisor Review content now opens with internal prep framing and sequences Manager Role Review, Data Confidence, Concentration Diagnostic, Scenario Sensitivity, Cash & Liquidity, Internal Follow-Ups, and a restrained demo note; narrative sections render as natural headings/body text without visible metadata wrappers; unsupported sections keep calm nontechnical placeholders.
