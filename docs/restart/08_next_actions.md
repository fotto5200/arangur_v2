...
- Next ask Frank/ChatGPT which internal/local product path to take next: add a second full revaluation scenario bundle, publish position-thesis assignment fixtures for one product-grade thesis lens, map revaluation-derived attribution outputs into report-element input fixtures, or improve specific report-content elements. Keep additional advisor UI/report-consumption expansion paused until that decision is explicit. Keep private-server deployment planning parked, and keep local UI micro-polish parked unless a blocking rehearsal issue appears. Do not implement charts, live data, production auth, production reporting, generated report history, full report library UI, Caddy, Lightsail, Cloudflare, DNS, or public deployment changes unless explicitly authorized.
...
- Acceptance criteria: Implemented baseline: `docs/architecture/attribution_methodology_and_calculation_audit_v1.md` defines attribution object definitions, benchmark hierarchy, effect formulas, current artifact audit, calculation gaps, recommended synthetic attribution convention, required synthetic data improvements, timing policy, future report implications, sequencing, and open questions. `python -m arangur.analytics.attribution_methodology_audit` emits a read-only local JSON summary that classifies current fields as arithmetic, supplied synthetic input, supplied formula allocation, or residual reconciler; records timing as unavailable; flags manager benchmark-basis gaps; and recommends Synthetic Attribution Calculation Inputs v1. Focused tests verify the audit document, command output, optional JSON output, fixed-share effect flagging, `Not separately measured` gap, timing policy, manager benchmark-basis gap, and no external API/secret markers.
...
- Next recommended path: Regenerate Attribution Mockups from Calculated Outputs v1. Use the calculated output pack as the source for local `attribution_v1` report input/view fixtures and Markdown product mockups, still without Advisor Preview/Populate/Present wiring or generated-report integration. Keep Timing Attribution, production/client attribution, scenario-versus-benchmark, probabilistic range, current-versus-proposed attribution, deployment planning, and external/live data out of scope unless separately authorized.
