...
- Next choose whether to add backend-free print/export formatting for the current local preview, create a richer local viewer around restored spec sets, or design the future backend persistence path for briefing spec sets. Do not implement persistence, charts, Docker/Postgres, live data, or production reporting unless explicitly authorized.
- Later add durable backend serialization/export of completed local Client Briefing Set and Advisor Review Set specs if desired, after local browser export shape and product requirements remain stable.
