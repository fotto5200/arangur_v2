...
- Connect valuation outputs to report element input contracts next: portfolio status, concentration, scenario impact by manager, cash generation, manager comparison, and data confidence elements should consume stable value-change and confidence packages. Prefer a contract/mapping batch before changing report generation or browser UI.
