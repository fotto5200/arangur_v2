...
- Next recommended path: map only the build-now lean report elements into report-element input/view fixtures under the audit constraints. Do not expose every revaluation field, do not add advisor UI controls, and include tests that prevent overstuffed report views. Alternative next choices are one thesis-lens assignment fixture, a third full-revaluation scenario, or targeted report-content cleanup after Frank chooses the report shape.
