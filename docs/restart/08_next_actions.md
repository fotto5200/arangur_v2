...
- Next recommended path: build Synthetic Attribution Calculation Inputs v1 before regenerating final attribution mockups. That tranche should add explicit selected-lens policy theme benchmark weights, actual-weight theme benchmark portfolio returns, per-theme asset lists/weights/returns/reference returns, manager benchmark type and construction basis, and residual tolerance so summary effects can be calculated instead of supplied as fixed-share allocations. Keep advisor UI/report wiring, generated-report integration, deployment planning, timing attribution, production/client attribution, and external data out of scope unless separately authorized.
