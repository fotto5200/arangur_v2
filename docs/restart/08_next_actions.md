...
- Next choose whether to serialize/export local Client Briefing Set and Advisor Review Set specs, generate a local preview from the current unsaved ordered set without persistence, or add a slightly richer static preview viewer around existing fixtures. Do not implement persistence, charts, Docker/Postgres, live data, or production reporting unless explicitly authorized.
