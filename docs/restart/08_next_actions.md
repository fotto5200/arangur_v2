...
- Next choose whether to do browser visual QA against the cleaned advisor preview flow, or resume Docker Compose private-demo packaging so Postgres-backed briefing spec-set save/load can be exercised in a local stack. Do not implement charts, live data, production auth, production reporting, or generated reports from saved spec sets unless explicitly authorized.
