...
- Next choose whether to resume Docker Compose private-demo packaging so Postgres-backed briefing spec-set save/load can be exercised in a local stack, or ask Frank/ChatGPT for the next bounded product prompt. Do not implement charts, live data, production auth, production reporting, or generated reports from saved spec sets unless explicitly authorized.
