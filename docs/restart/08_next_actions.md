...
- Next recommended path: build Calculated Synthetic Attribution Engine v1 from these inputs, still local-only and unwired, then regenerate attribution prerequisites/mockups only after calculated effects replace fixed-share supplied synthetic allocations. Alternate local paths remain targeted report-element content revision, broader v2 content polish, or Scenario by Lens aggregation. Advisor UI/report wiring and private-server deployment planning remain later separate decisions.
