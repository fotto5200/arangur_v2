...
- Next recommended path: Frank should review `docs/product/report_mockups/attribution_v1/` for keep/revise/kill and decide whether the next local product tranche is attribution content polish, broader v2 content polish, or Scenario by Lens aggregation using existing full-revaluation rows plus complete synthetic lens assignments. Advisor UI/report wiring, generated-report integration, and private-server deployment planning remain later separate decisions.
