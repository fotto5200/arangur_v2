...
## 19. Follow-Up: Advisor Policy Attribution v2 Benchmark Basis Polish

Advisor Policy Attribution v2 now includes a visible Benchmark Basis section in the primary manager/sleeve report and the concise totals note. The section names Synthetic Global Policy Benchmark and shows global benchmark return, neutral selected mandate basket return, target policy benchmark return, and actual allocation benchmark return. This keeps selected mandate effect interpretable without restoring the old bridge-grid summary or adding manager implementation back into the primary advisor policy report.
