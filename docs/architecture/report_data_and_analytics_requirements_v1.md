...
- lower-level synthetic attribution calculation inputs for theme benchmark policy weights, actual-weight theme benchmark portfolios, per-theme asset weights, per-asset returns/reference returns, and manager benchmark-basis metadata;
...
| Integrated Performance Attribution | Required | Required over period | Required for manager mode | Required for lens mode | Required | Required | Required | Required for flow-adjusted returns | Optional scenario overlay | Not required | Synthetic attribution prerequisite pack, local `attribution_v1` report inputs/views/mockups, and methodology audit now exist | Real/client benchmark maps, return history, holdings/trades, flows, approved methodology, lower-level synthetic calculation inputs, and manager benchmark-basis metadata | Value-add, timing, calculated-effect, or benchmark-relative claims beyond the synthetic prerequisite pack and audit |
...
- Future attribution fixtures should carry effect provenance and manager benchmark-basis metadata. Uncalculated effects such as fixed-share supplied synthetic allocations should be replaced with calculated lower-level inputs before durable product implementation, or explicitly labeled as supplied synthetic inputs in advisor/internal review.
