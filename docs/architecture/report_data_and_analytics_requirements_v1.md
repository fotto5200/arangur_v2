...
Synthetic Policy Allocation / Mandate Benchmark Pack v1 now satisfies those local synthetic prerequisite artifacts. Policy Allocation Review can use the explicit policy targets, actual manager allocation snapshot, and drift rows in a future mockup. Policy-Level Attribution has input scaffolding but still needs a calculated engine before it should be treated as a calculated report. Manager mandate attribution can now source explicit benchmark-basis rows, and equal-weight attribution remains diagnostic unless a policy artifact says otherwise.
