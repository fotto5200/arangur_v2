...
| Integrated Performance Attribution | Required | Required over period | Required for manager mode | Required for lens mode | Required | Required | Required | Required for flow-adjusted returns | Optional scenario overlay | Not required | Synthetic attribution prerequisite pack and local `attribution_v1` report inputs/views/mockups now exist | Real/client benchmark maps, return history, holdings/trades, flows, and approved methodology | Value-add, timing, or benchmark-relative claims beyond the synthetic prerequisite pack |
