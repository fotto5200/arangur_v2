...
- regenerated attribution report fixtures/mockups that consume the calculated synthetic attribution outputs;
...
| Integrated Performance Attribution | Required | Required over period | Required for manager mode | Required for lens mode | Required | Required | Required | Required for flow-adjusted returns | Optional scenario overlay | Not required | Synthetic attribution prerequisite pack, local `attribution_v1` report inputs/views/mockups, methodology audit, Synthetic Attribution Calculation Inputs v1, and Calculated Synthetic Attribution Engine v1 now exist | Real/client benchmark maps, return history, holdings/trades, flows, approved production methodology, regenerated report fixtures from calculated outputs, and production timing inputs | Value-add, timing, or benchmark-relative claims beyond the local synthetic prerequisite/input/calculated-output packs and audit |
...
- Future attribution fixtures should carry effect provenance and manager benchmark-basis metadata. Calculated Synthetic Attribution Engine v1 can now source those fields and calculated effects for the next local attribution mockup regeneration; final report fixtures should not continue using supplied fixed-share allocations.
