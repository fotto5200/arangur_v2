...
Update after Attribution Methodology and Calculation Audit v1: `docs/architecture/attribution_methodology_and_calculation_audit_v1.md` defines the attribution objects, benchmark hierarchy, calculation provenance, and gaps behind the synthetic attribution mockups. At audit time, summary selection/sizing effects were deterministic fixed-share synthetic allocations rather than lower-level calculated attribution effects, while portfolio return, global benchmark return, manager relative return, manager contribution, and lens-bucket relative contribution were arithmetic from local synthetic inputs. Later calculation-input, calculated-engine, and regeneration tranches replaced those fixed-share local mockup effects where supported.

Update after Synthetic Attribution Calculation Inputs v1: explicit lower-level local synthetic inputs now exist for the first calculated attribution lens, AI Adoption. The pack supplies selected-lens policy, theme benchmark weight states, theme benchmark return inputs, compact grouped asset inputs, explicit manager benchmark-basis metadata, residual policy, and timing-unavailable readiness. The subsequent calculated-output regeneration now consumes those inputs through the calculated engine where supported.

Update after Calculated Synthetic Attribution Engine v1: calculated local synthetic attribution outputs now exist under `data/simulation/attribution_calculated/synthetic_attribution_engine_v1/`. The output pack includes whole-portfolio calculated attribution, theme benchmark detail, theme asset detail, manager calculated attribution, and quality/readiness summaries. The regenerated attribution mockups now consume these calculated outputs rather than the older fixed-share supplied allocation fields where supported. This still does not wire Advisor Preview / Populate / Present / generated reports or make timing, production/client attribution, scenario-versus-benchmark, probabilistic range, proposed-portfolio, real-client, or deployment readiness available.
...
Update after Synthetic Policy Allocation / Mandate Benchmark Pack v1: local-only prerequisite artifacts now exist for explicit policy allocation, actual manager allocation, drift/tolerance rows, imputed-current baseline, manager mandate benchmarks, policy-level attribution input scaffolding, and equal-weight diagnostic classification. These inputs now feed Policy Allocation Review Mockups v1 and Policy-Level Attribution Calculation and Mockups v1. This update does not wire Advisor Preview / Populate / Present / generated reports.

Update after Policy Allocation Review Mockups v1: local-only policy allocation report inputs, view fixtures, and Markdown product-review mockups now exist under `data/simulation/report_element_inputs/policy_allocation_v1/`, `data/simulation/report_element_views/policy_allocation_v1/`, and `docs/product/report_mockups/policy_allocation_v1/`. The generated set covers Policy Allocation Review, Policy Allocation Drift Summary, Imputed Current Allocation Baseline, and Manager Mandate Benchmark Basis. The review report shows target versus actual manager/sleeve allocation with drift, tolerance, and status; the drift summary shows only review/material watch-list rows; and the imputed-current note suppresses drift attribution and is not a standard client report.

Update after Policy-Level Attribution Calculation and Mockups v1: local-only calculated outputs now exist under `data/simulation/policy_level_attribution/policy_level_attribution_engine_v1/`, with report inputs, views, and Markdown mockups under `policy_attribution_v1`. The generated set covers Policy-Level Attribution Summary, Policy-Level Manager Effect Detail, and Imputed Baseline Policy Attribution Variant. The summary bridge shows Global benchmark -> Target policy benchmark -> Actual allocation benchmark -> Actual portfolio; the manager detail shows target weight, actual weight, benchmark return, actual return, drift effect, and manager effect for all six managers; and the imputed baseline variant suppresses allocation drift effect while stating that current allocation is not proven ideal.

Update after Advisor Policy Attribution Redesign v2: local-only calculated outputs now exist under `data/simulation/policy_level_attribution/advisor_policy_attribution_engine_v2/`, with report inputs, views, and Markdown mockups under `policy_attribution_v2`. Advisor Policy Attribution by Manager/Sleeve is the primary product-review surface. It separates selected mandate effect, target weighting effect, and funding drift effect, labels actual manager return as context, excludes manager implementation from the primary advisor report, and supersedes Policy-Level Attribution Summary v1 as the primary review surface. The benchmark basis polish now makes the Synthetic Global Policy Benchmark and the global, neutral selected mandate basket, target policy benchmark, and actual allocation benchmark returns visible in the v2 mockups.
...
| Integrated Performance Attribution Summary | Regenerated in `attribution_v1` from calculated whole-portfolio output; keep real/client mode gated |
| Integrated Performance Attribution Detail | Regenerated in `attribution_v1` from calculated theme benchmark and asset detail outputs; timing remains unavailable |
| Manager Attribution Summary | Regenerated in `attribution_v1` from `manager_calculated_attribution_summary.json` |
| Lens-Based Performance Attribution | AI Adoption regenerated in `attribution_v1` from calculated theme detail; Energy Security is gated for calculated attribution until calculated outputs exist |
...
For synthetic demo mockups, the benchmark, period returns, weights/flows, and contribution inputs should come from Synthetic Attribution Prerequisite Pack v1 rather than being invented inside a report generator. Attribution Methodology and Calculation Audit v1 adds a stricter provenance rule: every visible attribution effect should be calculated from lower-level inputs or explicitly labeled as a supplied synthetic allocation with a documented reason. Synthetic Attribution Calculation Inputs v1 now supplies the lower-level selected-lens, theme benchmark weight/return, grouped asset, manager benchmark-basis, and residual inputs needed by the next calculated synthetic attribution engine. Real/client attribution remains gated until production return, benchmark, holding, flow, and methodology inputs exist.
...
Synthetic demo attribution mockups are now generated from calculated synthetic attribution outputs in `attribution_v1` where supported. The regenerated v1 mockups use global benchmark, theme benchmark, asset selection/sizing, and residual/unexplained wording; Manager Attribution uses Manager Benchmark Return; the AI Adoption lens report uses calculated theme rows. Energy Security calculated attribution is gated until calculated outputs exist for that lens. Timing remains unavailable unless a later tranche defines two clean portfolio states, flow treatment, and the required trade/flow history. Do not generate production/client attribution reports until historical return, benchmark, position history, holding/flow, and approved benchmark/proxy prerequisites exist.
...
| Integrated Performance Attribution Summary | For synthetic demo: Synthetic Attribution Prerequisite Pack v1 supplies benchmark, returns, weights/flows, decomposition inputs, and caveats; Attribution Methodology and Calculation Audit v1 documents which current effects are arithmetic versus supplied synthetic allocations; Synthetic Attribution Calculation Inputs v1 now supplies lower-level inputs for the next calculated engine. For real/client mode: production return, benchmark, holding, flow, and methodology inputs remain absent | Generated for local synthetic demo; keep real/client attribution gated and replace supplied allocation effects once calculated synthetic outputs exist |
| Integrated Performance Attribution Detail | For synthetic demo: calculated theme-benchmark detail rows, manager rows, and lower-level calculation inputs exist, with timing unavailable. For real/client mode: detailed production holdings/trades and reconciliation policy remain absent | Generated for local synthetic demo from calculated outputs; omit timing unless cleanly defined |
...
- use Synthetic Attribution Prerequisite Pack v1, Attribution Methodology and Calculation Audit v1, and Synthetic Attribution Calculation Inputs v1 for attribution work rather than inventing benchmark, return, weight, or decomposition values inside mockups;
- use the regenerated calculated attribution mockups as the product-review baseline before any final attribution wiring;
