...
Synthetic demo attribution mockups are now generated from the attribution prerequisite pack in `attribution_v1`. The content-polished v1 mockups use global benchmark, theme benchmark, asset selection/sizing, and residual/unexplained wording; Manager Attribution uses Manager Benchmark Return; lens-based reports use Theme Bucket, Portfolio Return, and Theme Benchmark Return. The methodology audit keeps those files useful for product review but flags the current summary effect components and manager component effects as not lower-level calculated. Synthetic Attribution Calculation Inputs v1 fills the input gap, but the mockups have not yet been regenerated from calculated engine outputs. Timing remains unavailable unless a later tranche defines two clean portfolio states, flow treatment, and the required trade/flow history. Do not generate production/client attribution reports until historical return, benchmark, position history, holding/flow, and approved benchmark/proxy prerequisites exist.
...
| Integrated Performance Attribution Detail | For synthetic demo: decomposition inputs, theme-benchmark detail rows, manager rows, and lower-level calculation inputs exist, with timing unavailable; component fields marked `Not separately measured` remain audit-recognized mockup gaps until a calculated engine regenerates outputs. For real/client mode: detailed production holdings/trades and reconciliation policy remain absent | Generated for local synthetic demo; omit timing unless cleanly defined and replace `Not separately measured` only after calculated outputs exist |
...
- prefer a separate Calculated Synthetic Attribution Engine v1 tranche before regenerating final attribution mockups;
