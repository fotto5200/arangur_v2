...
Populate a workflow with data lets the advisor choose a saved workflow, use the current synthetic demo data snapshot, and create an ephemeral demo generated report artifact without entering Builder/edit mode. The artifact is returned by `POST /api/generated-reports/demo-populate`, opened in a clean demo populated report view, and is not saved to a report library or history. Present / view reports remains staged as the next generated-report presentation/list workflow.
...
This UI does not generate charts or server-side preview files from the local set. It does not call the workflow-run API or show report-package links. Populate can create an ephemeral generated report artifact from a saved workflow and the current synthetic demo snapshot, but it does not persist generated artifacts, build a report library, or create report history. Backend briefing spec-set persistence is draft metadata only; durable production records and server-side report generation remain future work after product and data-model decisions are stable.
