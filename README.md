...
Open `http://127.0.0.1:8000/app/` to use the advisor demo. Advisor Home is **Briefings**, with one shared **Briefing Templates** list and a separate **Generated Reports** list. The four catalog-backed built-ins—Principal Briefing, Engaged Client Review, Advisor Oversight, and External Manager Story Translation—appear in the same template list as browser-created workflows. There is no separate conversation or report-manifest object.

Each template row can generate directly with the current synthetic snapshot, open in the existing builder, or duplicate/save as a custom template. Built-ins use the same `arangur.local_briefing_spec_set.v1` payload as custom workflows and cannot be overwritten or deleted. `POST /api/generated-reports/demo-populate` returns the generated artifact, which is saved under browser `localStorage` key `arangur.local_generated_reports.v1` and opened immediately at report 1 with Previous/Next navigation. See `docs/demo/advisor_report_workflows.md`.
...
The main advisor flow is Briefings -> generate or open a template -> generated report. Template construction remains secondary under Create or manage templates. Use `Preview Client Briefing` or `Preview Advisor Review` from the Builder to inspect a draft before saving; empty workflows are not previewed.
...
Developer / QA tools remain collapsed and secondary. They contain local workflow JSON copy/download/restore, technical local export details, optional backend save/load test controls, and QA reference preview links.
