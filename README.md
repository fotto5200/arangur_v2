...
Populate a workflow with data lets the advisor choose a saved workflow, bind it to the current synthetic Northstar demo data snapshot, and create a local demo populated preview without entering Builder/edit mode. Full dated report generation and persistence from saved workflows are not implemented yet. Present / view reports lets the advisor choose a saved workflow and open a clean Presentation view using the existing browser preview renderer without showing builder controls, the report element finder, JSON tools, or backend save/load controls.
...
Named saved workflows are browser-local draft workflow definitions stored in `localStorage` under `arangur.local_named_briefing_workflows.v1`. Saved workflow records preserve a human-readable `display_name` such as `Report Sequence One` separately from the internal `workflow_id` / `storage_key`, wrap the same local spec-set payload shape used for JSON transfer, can be saved, opened to edit, copied with readable names, deleted, populated with current demo data, and opened in Presentation view from the Advisor Home / Builder flow. They do not create backend records or generated report history.
