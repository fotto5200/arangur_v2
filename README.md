...
Advisor Home also presents four catalog-backed report journeys above those existing workflow tools: Principal Briefing, Engaged Client Review, Advisor Oversight, and External Manager Story Translation. Each journey preserves the committed report order, links only to available allowlisted previews, and leaves gated or deferred stages unlinked. See `docs/demo/advisor_report_workflows.md` for sourcing, fallback behavior, and smoke instructions.
