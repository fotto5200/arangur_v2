...
The ordinary advisor home is organized around four activities: **Prepare a New Briefing Plan**, **Work with Existing Plans or Briefings**, **Present a Dated Briefing**, and **Ask Arangur**. Prepare has only Build from Scratch and Start from a Template. Existing opens one contextual Draft/Plan/Dated Briefing library. Present opens one searchable Dated Briefing list. Ask Arangur opens one deterministic local prompt. Recent Work and Recent Dated Briefings remain small shortcuts; Developer / QA remains subordinate.

The ordinary lifecycle is **Briefing Plan → Create Dated Briefing → View Dated Briefing → Present Dated Briefing**. Every populated presentable Briefing Section is selected by default; protected internal and unavailable material remains excluded. Choose Sections is optional and preserves the advisor's include/exclude/order choices. A Dated Briefing can be presented without formal Advisor Review, Preview, Prepare-for-Presentation, or Ready-to-Present gates. Old status/eligibility fields remain compatible Developer / QA diagnostics. Universal Explain/Verify actions are removed; More Detail appears only for genuine explicit supporting content, while exact attribution tables remain directly visible.

Canonical contract: `docs/product/arangur_advisor_workflow_concept_simplification_v1.md`. Local QA: `docs/demo/arangur_advisor_workflow_concept_simplification_qa_v1.md`. The advanced builder, immutable Dated Briefings, history, selected sequence/progress, external-story governance, legacy normalization, and Developer / QA remain available.
