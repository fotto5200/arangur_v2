...
Open `http://127.0.0.1:8000/app/` to use the Report Element Finder / Composer prototype. It lands on Advisor Home as a product workflow hub for designing or editing briefing workflows, naming and reopening browser-local saved workflows, previewing the current Client Briefing or Advisor Review surface, and keeping technical handoff tools under Developer / QA tools. The static app uses hash routes such as `#home`, `#builder`, `#preview/client`, `#preview/advisor`, and `#advanced` so browser Back moves from Preview to Builder and from Builder to Advisor Home.
