...
Open `http://127.0.0.1:8000/app/` to use the Report Element Finder / Composer prototype. It lands on Advisor Home with exactly four product choices: Create a new workflow, Work with an existing workflow, Populate a workflow with data, and Present / view reports. The static app uses hash routes such as `#home`, `#builder`, `#preview/client`, `#preview/advisor`, and `#advanced` so browser Back moves from Preview to Builder and from Builder to Advisor Home.

Create a new workflow reveals only the workflow-type choices: New Client Briefing Workflow and New Advisor Review Workflow. Work with an existing workflow is the single Advisor Home surface for browser-local saved workflows, including open, copy, and delete.

Populate a workflow with data is currently a staged flow: choose a saved workflow, use the current demo data snapshot, then open the builder preview. Full dated report generation from saved workflows is not implemented yet. Present / view reports is also staged: generated report presentation is a next workflow, and current advisor use should open a workflow and use Preview Client Briefing or Preview Advisor Review from the builder.
...
The main advisor flow is Advisor Home -> Builder -> Preview -> edit. Use `Preview Client Briefing` or `Preview Advisor Review` from Advisor Home or the workflow panels to open a focused preview surface for the current ordered workflow. Empty workflows are not previewed; the builder asks for at least one client briefing or advisor review element first.
...
Named saved workflows are browser-local draft workflow definitions stored in `localStorage` under `arangur.local_named_briefing_workflows.v1`. They wrap the same local spec-set payload shape used for JSON transfer, can be saved, opened, copied, and deleted from the Advisor Home / Builder flow, and do not create backend records or generated report history.
