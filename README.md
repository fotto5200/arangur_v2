...
The workbench now also uses immediate Included/Available selection feedback, meaningful Draft/Plan/Dated Briefing identity, an explicit Dated Briefing → Reviewed → Ready-to-Present model, shared Preview/Presentation eligibility, object-bound Presentation exit/resume, explicit comparison, and predictable return routes. Audience now supplies recommended defaults rather than a prohibition. After review, **Prepare for Presentation** lets the advisor include, exclude, and reorder any populated presentable Briefing Section, including advisor-oriented material. Protected internal and unavailable material cannot be selected. Reviewed plus at least one saved selected presentable section plus no blocking condition is Preview eligible; Ready to Present is also Presentation eligible; Resume requires actual saved progress. Preview, Presentation, Choose, Start, Resume, Find, home shortcuts, and the reader use the same normalized browser-local record and exact saved sequence.

Canonical contracts: `docs/product/arangur_advisor_workbench_information_architecture_v1.md`, `docs/product/arangur_advisor_workbench_state_navigation_correction_v1.md`, `docs/product/arangur_dated_briefing_presentation_discovery_fix_v1.md`, and `docs/product/arangur_presentation_section_selection_and_audience_flexibility_v1.md`. Local walkthroughs include `docs/demo/arangur_presentation_section_selection_qa_v1.md`.
