...
CROSS_SCENARIO_METHODOLOGY = "full_portfolio_revaluation_cross_scenario_summary"
...
SCENARIO_ATTRIBUTION_KINDS = (
    "manager_revaluation_attribution",
    "account_revaluation_attribution",
    "sleeve_revaluation_attribution",
    "coverage_revaluation_attribution",
    "confidence_revaluation_attribution",
    "theme_revaluation_attribution",
)

OUTPUT_FILES = {
    **{
        f"{kind}_{scenario_id}": f"{kind}_{scenario_id}.json"
        for scenario_id in SUPPORTED_SCENARIO_IDS
        for kind in SCENARIO_ATTRIBUTION_KINDS
    },
    "thesis_bucket_revaluation_attribution_readiness": "thesis_bucket_revaluation_attribution_readiness.json",
    "cross_scenario_revaluation_summary": "cross_scenario_revaluation_summary.json",
...
def input_files_for_scenario(scenario_id: str) -> dict[str, str]:
    ...
    return {
        ...
        "position_value_comparison": f"position_value_comparison_{scenario_id}.json",
        "portfolio_revaluation_summary": f"portfolio_revaluation_summary_{scenario_id}.json",
        "valuation_coverage_manifest": f"valuation_coverage_manifest_{scenario_id}.json",
        "position_catalog": INPUT_FILES["position_catalog"],
        "instrument_catalog": INPUT_FILES["instrument_catalog"],
    }
...
def generate_revaluation_attribution_outputs(
    *,
    ...
    contexts = {
        scenario_id: load_attribution_context(
    ...
    for scenario_id, context in contexts.items():
        outputs[f"manager_revaluation_attribution_{scenario_id}"] = build_group_attribution(
            context, group_key="manager_id", group_kind="manager"
        )
        outputs[f"account_revaluation_attribution_{scenario_id}"] = build_group_attribution(
            context, group_key="account_id", group_kind="account"
        )
        outputs[f"sleeve_revaluation_attribution_{scenario_id}"] = build_group_attribution(
            context, group_key="sleeve_id", group_kind="sleeve"
        )
        outputs[f"coverage_revaluation_attribution_{scenario_id}"] = build_bucket_attribution(
            context, bucket_key="scenario_coverage_status", bucket_kind="coverage"
        )
        outputs[f"confidence_revaluation_attribution_{scenario_id}"] = build_bucket_attribution(
            context, bucket_key="confidence", bucket_kind="confidence"
        )
        outputs[f"theme_revaluation_attribution_{scenario_id}"] = build_theme_attribution(context)

    primary_context = contexts[scenario_ids[0]]
    outputs["thesis_bucket_revaluation_attribution_readiness"] = build_thesis_bucket_readiness(contexts)
    outputs["cross_scenario_revaluation_summary"] = build_cross_scenario_summary(contexts)
    outputs["cross_scenario_revaluation_readiness"] = build_cross_scenario_readiness(contexts)
    outputs["revaluation_attribution_index"] = build_attribution_index(primary_context, contexts, outputs)
...
def load_attribution_context(
    *,
    ...
    input_files = input_files_for_scenario(scenario_id)
    inputs = {
    ...
    return _base_output(
        ...
        extra={
            ...
            "caveats": [
                ...
                "This artifact supersedes legacy proof impact views only for the full-revaluation scenario shown here.",
            ],
        },
    )


def build_thesis_bucket_readiness(contexts: dict[str, dict[str, Any]]) -> dict[str, Any]:
    context = contexts[SCENARIO_ID]
    artifacts = sorted(
        {
            artifact
            for scenario_context in contexts.values()
            for artifact in scenario_context["thesis_assignment_artifacts"]
        }
    )
    return _base_output(
        context,
...
def build_cross_scenario_summary(contexts: dict[str, dict[str, Any]]) -> dict[str, Any]:
    scenario_rows = [_cross_scenario_row(context) for context in contexts.values()]
    scenario_rows.sort(key=lambda row: row["impact"])
    worst = scenario_rows[0]
    best = max(scenario_rows, key=lambda row: row["impact"])
    all_rows = [
        {**row, "scenario_id": scenario_id}
        for scenario_id, context in contexts.items()
        for row in context["rows"]
    ]
    manager_rows = _cross_group_rows(all_rows, "manager_id", contexts[SCENARIO_ID]["manager_labels"])
    sleeve_rows = _cross_group_rows(all_rows, "sleeve_id", contexts[SCENARIO_ID]["sleeve_labels"])
    primary = contexts[SCENARIO_ID]
    return {
        "schema_version": "cross_scenario_revaluation_summary.v1",
        "generated_at": GENERATED_AT,
        "methodology": CROSS_SCENARIO_METHODOLOGY,
        "synthetic_data": True,
        "scenario_count": len(scenario_rows),
        "scenarios_included": [row["scenario_id"] for row in sorted(scenario_rows, key=lambda row: row["scenario_id"])],
        "base_portfolio_value": primary["portfolio_summary"]["base_portfolio_value"],
        "scenarios": sorted(scenario_rows, key=lambda row: row["scenario_id"]),
        "worst_scenario_by_impact": worst,
        "best_scenario_by_impact": best,
        "top_positions_appearing_across_scenarios_by_absolute_impact": _cross_position_rows(all_rows),
        "managers_most_affected_across_scenarios": manager_rows,
        "sleeves_most_affected_across_scenarios": sleeve_rows,
        "source_bundle_references": [
            {
                "scenario_id": context["bundle_manifest"]["scenario_id"],
                "bundle_manifest": _manifest_path_for_scenario(scenario_id),
                "position_value_comparison": f"data/simulation/revaluation/{context['input_files']['position_value_comparison']}",
            }
            for scenario_id, context in sorted(contexts.items())
        ],
        "caveats": [
            "Derived only from full revaluation comparison outputs.",
            "Legacy proof scenario impact outputs are not mixed into this summary.",
            "Only two synthetic full-revaluation scenarios are available in this checkpoint.",
        ],
    }


def build_cross_scenario_readiness(contexts: dict[str, dict[str, Any]]) -> dict[str, Any]:
    context = contexts[SCENARIO_ID]
    return _base_output(
        context,
        schema_version="cross_scenario_revaluation_readiness.v1",
        extra={
            "status": "cross_scenario_summary_available",
            "available_full_revaluation_scenario_count": len(contexts),
            "available_scenario_ids": sorted(contexts),
            "cross_scenario_summary_file": "data/simulation/revaluation/attribution/cross_scenario_revaluation_summary.json",
            "readiness_notes": [
                "A two-scenario full-revaluation summary is now available.",
                "Additional scenarios are still needed before calling this a broad resilience view.",
                "Existing legacy proof outputs may remain useful as reference, but they are not mixed into this full-revaluation summary.",
            ],
            "caveats": [
                "Only AI / Chip Selloff and Rate Shock have committed full revaluation bundles in this checkpoint.",
            ],
        },
    )
    ...
    scenario_rows = [
        {
            ...
            "bundle_manifest": _manifest_path_for_scenario(scenario_id),
            "attribution_outputs": [
                f"data/simulation/revaluation/attribution/{OUTPUT_FILES[f'{kind}_{scenario_id}']}"
                for kind in SCENARIO_ATTRIBUTION_KINDS
            ],
        }
        for scenario_id in sorted(contexts)
    ]
    return _base_output(
        context,
        ...
        extra={
            ...
            "source_manifests": [
                ...
                "data/simulation/revaluation/valuation_coverage_manifest_rate_shock.json",
            ],
            "source_inputs": [
                "data/simulation/revaluation/position_value_comparison_ai_chip_selloff.json",
                "data/simulation/revaluation/position_value_comparison_rate_shock.json",
...
def _base_output(context: dict[str, Any], *, schema_version: str, extra: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_revaluation_manifest": _manifest_path_for_scenario(bundle["scenario_id"]),
...
def _cross_scenario_row(context: dict[str, Any]) -> dict[str, Any]:
    summary = context["portfolio_summary"]
    bundle = context["bundle_manifest"]
    return {
        "scenario_id": bundle["scenario_id"],
        ...
        "impact_percent": summary["impact_percent"],
        ...
        "source_bundle_manifest": _manifest_path_for_scenario(bundle["scenario_id"]),
    }


def _cross_position_rows(rows: list[dict[str, Any]], *, limit: int = 10) -> list[dict[str, Any]]:
    grouped: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for row in rows:
    ...
    for position_id, position_rows in grouped.items():
        ...
        scenario_impacts = [
            ...
            for row in sorted(position_rows, key=lambda item: item["scenario_id"])
        ]
        output_rows.append(
            {
                "position_id": position_id,
                "display_name": first["display_name"],
                "manager_id": first.get("manager_id"),
                "sleeve_id": first.get("sleeve_id"),
                "scenario_count": len(position_rows),
                "total_absolute_impact": _round_money(sum(abs(float(row["value_change"])) for row in position_rows)),
                "worst_impact": min(float(row["value_change"]) for row in position_rows),
                "best_impact": max(float(row["value_change"]) for row in position_rows),
                "scenario_impacts": scenario_impacts,
            }
        )
    output_rows.sort(key=lambda row: (row["total_absolute_impact"], row["position_id"]), reverse=True)
    return output_rows[:limit]
...
def _cross_group_rows(
    ...
    *,
    ...
    output_rows = []
    for group_id, group_rows in grouped.items():
        scenario_impacts: dict[str, float] = defaultdict(float)
        for row in group_rows:
            scenario_impacts[row["scenario_id"]] += float(row["value_change"])
        output_rows.append(
            {
                "group_id": group_id,
                "display_name": labels.get(group_id, group_id),
                "scenario_count": len(scenario_impacts),
                "total_absolute_impact": _round_money(sum(abs(value) for value in scenario_impacts.values())),
                "scenario_impacts": [
                    {
                        "scenario_id": scenario_id,
                        "impact": _round_money(impact),
                    }
                    for scenario_id, impact in sorted(scenario_impacts.items())
                ],
            }
        )
    output_rows.sort(key=lambda row: (row["total_absolute_impact"], row["group_id"]), reverse=True)
    return output_rows[:limit]
...
def _manifest_path_for_scenario(scenario_id: str) -> str:
    _validate_scenario_id(scenario_id)
    filename = "revaluation_bundle_manifest.json" if scenario_id == SCENARIO_ID else f"revaluation_bundle_manifest_{scenario_id}.json"
    return f"data/simulation/revaluation/{filename}"
...
def _validate_scenario_id(scenario_id: str) -> None:
    if scenario_id not in SUPPORTED_SCENARIO_IDS:
        ...
        raise ValueError(f"Unsupported revaluation attribution scenario: {scenario_id}. Supported scenarios: {supported}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    index = outputs["revaluation_attribution_index"]

    print("Full-revaluation attribution outputs generated")
    print(f"Scenarios: {', '.join(index['scenario_ids'])}")
    for scenario_id in index["scenario_ids"]:
        manager = outputs[f"manager_revaluation_attribution_{scenario_id}"]
        account = outputs[f"account_revaluation_attribution_{scenario_id}"]
        sleeve = outputs[f"sleeve_revaluation_attribution_{scenario_id}"]
        coverage = outputs[f"coverage_revaluation_attribution_{scenario_id}"]
        confidence = outputs[f"confidence_revaluation_attribution_{scenario_id}"]
        print(
            f"{scenario_id}: managers={manager['group_count']}, accounts={account['group_count']}, "
            f"sleeves={sleeve['group_count']}, coverage_buckets={coverage['bucket_count']}, "
            f"confidence_buckets={confidence['bucket_count']}"
        )
